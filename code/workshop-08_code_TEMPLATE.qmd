...
drought_exp_clean <- drought_exp |> # creating a clean dataframe starting with drought_exp object
  
  clean_names() |> 
  # adding new column for scientific names
  ...
  # New column with full names for water treatment
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) |> 
  # making sure species_name is a factor and reordering the levels
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # making sure that water treatent is a factor and reording levels
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = expression(Specific~leaf~area~(cm^2/g)),
       y = "Total mass (g)",
       title = "As SLA increases, total mass tends to increase \n as well (positive)") #\n makes a linebreak
```
...
plot(model1) #keep doing the others
```

## Model selection

```{r model-selection}
AICc( #aic, simplest that explains the most variation
  model0,
  model1,
  model2,
  model3,
  model4, #model 4 is the best
  model5,
  model6,
  model7
) |> 
...
**What is the best model?**
 ...
 The best model that explains total plant mass includes water treatment and species.
 (F(7, 62) = 27.08, p<0.001, $\alpha$ = 0.05), but not SLA
...
Across water treatments, all species have different (and higher) masses than _Lotus scroparius_ except _Penstemon centranthifolius_


# 5. Model predictions

```{r model-predictions}
model4_prediction <- ggpredict(
  model4,
  terms = c("species_name", "water_treatment") #listing predictors
  
) |> 
  #rename columns (treating this like a dataframe)
...
ggplot(data = drought_exp_clean,
  ...
  # in the final model
  geom_pointrange(data = model4_prediction,
                  aes(y = predicted,
                      ymin = conf.low,
                      ymax = conf.high)) +
  # manually setting colors
